...
class StepWindow:
    ...
    @property
    def stem(self) -> str:
        return f"step_{self.step:02d}_{self.cpu}"
...
def save_step(out_dir: Path, step: StepWindow, t: np.ndarray, client: np.ndarray, server: np.ndarray) -> Path:
    ...
    np.savez_compressed(
        ...
        cpu_millis=np.int32(parse_cpu_millis(step.cpu)),
        cpu=np.asarray(step.cpu),
    ...
    for csv_path, out_dir in jobs:
        ...
        if summary_path.is_file() and not args.force:
            ...
            if summary_rows:
                ...
                np.savez_compressed(
                    summary_path, cpu=np.asarray(cpu_labels), **packed
                )
