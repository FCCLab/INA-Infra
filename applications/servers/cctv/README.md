...
flowchart TD
  ...
  subgraph server_pod [Regional Cluster - application-cctv Pod]
    ...
    Ui["frontend container: Nginx :80 (Multus 10.1.137.211) React Video Wall"]
...
1. **`cctv`** (Backend) — GStreamer RECORD Ingest on Multus (`10.1.137.211:8554` with static MAC `02:0a:89:a0:00:01`), YOLOv8 inference workers (BGR native pipeline), FastAPI (`/snapshot`, `/api`), and Prometheus exporter (`:9102`).
...
3. **`frontend`** (Web Gateway) — Nginx serving the React Video Wall SPA on Multus `10.1.137.211:80`, reverse-proxying API, snapshots, and HLS streaming with on-demand `IntersectionObserver` video mounting.
