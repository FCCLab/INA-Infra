#!/usr/bin/env bash
# Build amd64 Physical AI dashboard sidecar and push to the lab registry.
set -euo pipefail
...
docker build --platform linux/amd64 -f Dockerfile.dashboard -t "${LOCAL_TAG}" -t "${REMOTE_TAG}" .
