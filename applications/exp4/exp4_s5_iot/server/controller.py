...
# Static DL targets so fan-out works with UL disabled (comma-separated device ids).
DL_DEVICE_IDS = [
    d.strip()
...
MQTT_TOPIC_PREFIX = _env("MQTT_TOPIC_PREFIX", "slice_5")
UL_TOPIC_PREFIX = f"{MQTT_TOPIC_PREFIX}/ul/"
LATENCY_TOPIC_PREFIX = f"{MQTT_TOPIC_PREFIX}/latency/"
PROBE_TOPIC_PREFIX = f"{MQTT_TOPIC_PREFIX}/probe/"
PROBE_ACK_PREFIX = f"{MQTT_TOPIC_PREFIX}/probe-ack/"
...
class Controller:
    ...
    def _on_connect(self, client, _userdata, _flags, reason_code, _properties):
        ...
        if is_reconnect:
            M.reconnects.inc()
        M.connected.set(1)
        client.subscribe(f"{MQTT_TOPIC_PREFIX}/ul/#", qos=MQTT_QOS)
        client.subscribe(f"{MQTT_TOPIC_PREFIX}/probe/#", qos=MQTT_QOS)
    ...
    def _live_devices(self) -> list[str]:
        ...
        # Always include static DL targets (DL-only mode: no UL discovery).
        for d in DL_DEVICE_IDS:
            if d not in live:
                live.append(d)
