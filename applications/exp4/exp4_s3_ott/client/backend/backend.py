...
class MosaicQualityIn(BaseModel):
    quality: str = Field(default_factory=lambda: os.environ.get("OTT_PLAY_QUALITY", "4k"))
...
MOSAIC_COUNT = max(1, int(os.environ.get("OTT_MOSAIC_COUNT", "4") or "4"))
MOSAIC_DEFAULT = os.environ.get("OTT_MOSAIC", "1") not in ("0", "false", "False")
_FALLBACK_MOSAIC = (
    {"id": "channel_1", "youtube_id": "yc31e3h8Ehk", "name": "24h Underwater 4K"},
    {"id": "channel_2", "youtube_id": "JTEQQrtXrYM", "name": "Norway Nature 4K"},
    {"id": "channel_3", "youtube_id": "3xPkwNu2o8g", "name": "Splendors of Nature 4K"},
    {"id": "channel_4", "youtube_id": "SYyvx6GE2UI", "name": "Nature Europe 4K"},
)
...
def _watchdog_reload_page(video_id: str, youtube_id: str, quality: str) -> None:
    """Navigate Chromium back to the assigned watch URL (or full mosaic)."""
    with _lock:
        mosaic = bool(_state.get("mosaic"))
        slots = list(_state.get("mosaic_slots") or [])
    if mosaic and len(slots) > 1:
        for i, slot in enumerate(slots):
            if slot.get("enabled", True):
                chrome_ctl.play_slot(slot, quality=quality, index=i)
            else:
                chrome_ctl.blank_slot(slot, index=i)
        with _lock:
            _state["mosaic_slots"] = slots
            _state["streaming_enabled"] = True
        return
    ...
    page = chrome_ctl._pick_page(prefer_video_id=yt_id)
    if page:
        chrome_ctl.navigate_page(page, play_url)
    else:
    ...
    while True:
        ...
        try:
            with _lock:
                ...
                mosaic = bool(_state.get("mosaic"))
                mosaic_slots = list(_state.get("mosaic_slots") or [])
            ...
            if not enabled:
                ...
                if mosaic and mosaic_slots:
                    # Keep the four mosaic tabs; Stop / per-tab Off already blanked them.
                    bad_ticks = 0
                else:
                ...
                action = "waiting_cdp"
                bad_ticks += 1
            elif mosaic and mosaic_slots:
                active = [s for s in mosaic_slots if s.get("enabled", True)]
                playing_n = 0
                ads_n = 0
                loaded_n = 0
                res = {}
                if not active:
                    ...
                    with _cdp_lock:
                        healed = chrome_ctl.heal_mosaic(mosaic_slots, quality=quality)
                    playing_n = sum(1 for r in healed if r.get("playing"))
                    ads_n = sum(1 for r in healed if r.get("ad_skipped") or r.get("ad_detected"))
                    loaded_n = sum(1 for r in healed if r.get("page_loaded"))
                    res = next((r for r in healed if r.get("enabled")), healed[0] if healed else {})
                    if ads_n:
                        action = "ad_skipped" if any(r.get("ad_skipped") for r in healed) else "ad"
                        bad_ticks = 0
                        if action == "ad_skipped" and prev_action != "ad_skipped":
                            event_msg = f"Advertisement skipped on {ads_n}/{len(active)} on mosaic tabs"
                    elif playing_n >= max(1, len(active) - 1):
                        action = "playing_ok"
                        bad_ticks = 0
                    else:
                        bad_ticks += 1
                        action = f"mosaic_{playing_n}/{len(active)}"
                        need_reload = (
                            bad_ticks >= WATCHDOG_RELOAD_AFTER
                            and (now - last_reload) >= WATCHDOG_RELOAD_COOLDOWN_S
                        )
                        if need_reload:
                            try:
                                with _cdp_lock:
                                    for i, slot in enumerate(mosaic_slots):
                                        if slot.get("enabled", True):
                                            chrome_ctl.play_slot(slot, quality=quality, index=i)
                                last_reload = now
                                bad_ticks = 0
                                action = "reloaded"
                                event_msg = (
                                    f"Watchdog reloaded {len(active)} on / {len(mosaic_slots)} tabs "
                                    f"(playing={playing_n}/{len(active)} loaded={loaded_n})"
                                )
                            except Exception as exc:
                                action = f"reload_failed: {exc}"
                                event_msg = f"Watchdog mosaic reload failed: {exc}"
                with _lock:
                    _state["watchdog_last_check"] = now
                    _state["watchdog_last_action"] = action
                    _state["mosaic_playing"] = playing_n
                    ...
                    if action == "ad_skipped" and prev_action != "ad_skipped":
                        _state["ads_skipped_count"] = int(_state.get("ads_skipped_count") or 0) + ads_n
                ...
                prev_action = action
                time.sleep(0.75 if action in ("ad", "ad_skipped") else period)
                continue
            else:
                with _cdp_lock:
...
def _slot_from_video(item: dict) -> dict:
    ...
    if youtube_id:
        ...
        if vq:
            play_url = f"{play_url}&vq={vq}"
    return {
        "video_id": video_id or youtube_id,
        "name": item.get("name") or item.get("title") or video_id,
...
def _mosaic_slots_from_catalog() -> List[dict]:
    videos: List[dict] = []
    try:
        data = _http_json("GET", "/api/v1/videos", timeout=8.0)
        videos = list(data.get("videos") or [])
    except Exception as exc:
        logger.warning("mosaic catalog fetch failed: %s", exc)
        videos = list(_video_cache)
    if not videos:
        videos = [dict(v) for v in _FALLBACK_MOSAIC]
    slots: List[dict] = []
    for i in range(MOSAIC_COUNT):
        item = videos[i % len(videos)]
        slot = _slot_from_video(item)
        slot["index"] = i
        if slot.get("youtube_id"):
            slots.append(slot)
    if not slots:
        raise RuntimeError("No YouTube videos in catalog for 4K mosaic")
    return slots


def _ensure_pdu_socks() -> tuple[str, str]:
...
def _sync_mosaic_fields() -> None:
    """Update derived mosaic fields. Caller holds _lock."""
    slots = list(_state.get("mosaic_slots") or [])
    enabled = [s for s in slots if s.get("enabled", True)]
    ids = [str(s.get("youtube_id") or "") for s in slots if s.get("youtube_id")]
    on_ids = [str(s.get("youtube_id") or "") for s in enabled if s.get("youtube_id")]
    _state["mosaic"] = True
    _state["mosaic_playing"] = len(enabled)
    _state["youtube_ids"] = ids
    _state["youtube_id"] = (on_ids[0] if on_ids else (ids[0] if ids else ""))
    if enabled:
        primary = enabled[0]
        _state["selected_video_id"] = primary.get("video_id") or _state.get("selected_video_id") or ""
        _state["youtube_url"] = primary.get("youtube_url") or ""
        _state["embed_url"] = primary.get("play_url") or ""
        _state["chrome_url"] = primary.get("play_url") or ""


def _ensure_mosaic_session(quality: str | None = None) -> List[dict]:
    with _lock:
        slots = list(_state.get("mosaic_slots") or [])
        mosaic = bool(_state.get("mosaic"))
    if mosaic and len(slots) >= 1:
        return slots
    _play_chromium_mosaic(quality=quality)
    with _lock:
        return list(_state.get("mosaic_slots") or [])
...
def _set_mosaic_tab(
    *,
    ...
    slots = _ensure_mosaic_session(quality=q_in)
    if not slots:
        raise RuntimeError("no mosaic tabs")
    idx: Optional[int] = None
    if index is not None:
        idx = int(index)
    else:
        needle = (video_id or youtube_id or "").strip()
        for i, slot in enumerate(slots):
            if needle and needle in (
                str(slot.get("video_id") or ""),
                str(slot.get("youtube_id") or ""),
            ):
                idx = i
                break
    if idx is None or idx < 0 or idx >= len(slots):
        raise RuntimeError("unknown mosaic tab")
    slot = slots[idx]
    iface, pdu_ip = _ensure_pdu_socks()
    with _cdp_lock:
        if enabled:
            chrome_ctl.play_slot(slot, quality=yt_q, index=idx)
        else:
            chrome_ctl.blank_slot(slot, index=idx)
    with _lock:
        slots[idx] = slot
        _state["mosaic_slots"] = slots
        ...
        _state["streaming_enabled"] = True
        _sync_mosaic_fields()
    label = slot.get("name") or slot.get("youtube_id") or f"tab {idx + 1}"
    _log_event(
        "control",
        f"Tab {idx + 1} {'ON' if enabled else 'OFF'} — {label} via SOCKS/PDU {iface}/{pdu_ip}",
    )
    return {
        "ok": True,
        "index": idx,
        ...
        "tabs": len(slots),
        "on": sum(1 for s in slots if s.get("enabled", True)),
        "slots": slots,
        "quality": q_in,
        "yt_quality": yt_q,
    }
...
def _reapply_mosaic_quality(quality: str | None = None) -> Dict[str, Any]:
    q_in = (quality or DEFAULT_PLAY_QUALITY or "4k").strip() or "4k"
    yt_q = chrome_ctl.normalize_quality(q_in)
    with _lock:
        slots = list(_state.get("mosaic_slots") or [])
    if not slots:
        return _play_chromium_mosaic(quality=q_in)
    _ensure_pdu_socks()
    with _cdp_lock:
        for i, slot in enumerate(slots):
            if slot.get("enabled", True):
                chrome_ctl.play_slot(slot, quality=yt_q, index=i)
    with _lock:
        _state["mosaic_slots"] = slots
        _state["play_quality"] = q_in
        _state["yt_quality"] = yt_q
        _state["streaming_enabled"] = True
        _sync_mosaic_fields()
    on_n = sum(1 for s in slots if s.get("enabled", True))
    _log_event("control", f"Mosaic quality {q_in} applied to {on_n}/{len(slots)} on tabs")
    return {
        "ok": True,
        "quality": q_in,
        "yt_quality": yt_q,
...
def _play_chromium_mosaic(quality: str | None = None) -> Dict[str, Any]:
    """Play MOSAIC_COUNT catalog videos at 4K as tabs in one Chromium."""
    q_in = (quality or DEFAULT_PLAY_QUALITY or "4k").strip() or "4k"
    yt_q = chrome_ctl.normalize_quality(q_in)
    slots = _mosaic_slots_from_catalog()
    iface, pdu_ip = _ensure_pdu_socks()
    try:
        with _cdp_lock:
            nav = chrome_ctl.play_mosaic(slots, quality=yt_q)
    except Exception as exc:
        raise RuntimeError(f"Chromium mosaic navigate failed: {exc}") from exc

    ids = [s["youtube_id"] for s in slots]
    video_ids = [s["video_id"] for s in slots]
    ...
    with _lock:
        _state["mosaic"] = True
        _state["mosaic_slots"] = slots
        _state["mosaic_playing"] = len(slots)
        _state["selected_video_id"] = primary["video_id"]
        _state["assigned_channel"] = primary["video_id"]
        _state["play_mode"] = "chromium_5g"
        _state["youtube_id"] = ids[0]
        _state["youtube_ids"] = ids
        _state["youtube_url"] = primary["youtube_url"]
        _state["embed_url"] = primary["play_url"]
        _state["chrome_url"] = primary["play_url"]
    ...
    _log_event(
        "play",
        f"Chromium 1 window / {len(slots)} tabs ×{q_in} → {','.join(ids)} via SOCKS/PDU {iface}/{pdu_ip}",
    )
    return {
        "ok": True,
        "mosaic": True,
...
def _play_chromium(video_id: str, quality: str | None = None) -> Dict[str, Any]:
    ...
    with _lock:
        ...
        _state["mosaic"] = False
        _state["mosaic_slots"] = []
        _state["youtube_ids"] = [youtube_id] if youtube_id else []
        _state["hls_url"] = ""
        _state["rtsp_url"] = ""
        _state["whep_url"] = ""
        _state["streaming_enabled"] = True
...
def _chrome_autostart_loop() -> None:
    ...
    while True:
        try:
            ...
            chrome = chrome_ctl.status()
            cur = str(chrome.get("url") or "")
            yt_pages = int(chrome.get("youtube_pages") or 0)
            with _lock:
...
def play_video(req: PlayIn):
    """Play 4×4K mosaic by default; a video_id with mosaic=false plays one window."""
    try:
        want_mosaic = MOSAIC_DEFAULT if req.mosaic is None else bool(req.mosaic)
        if want_mosaic or not (req.video_id or "").strip() or (req.video_id or "").strip().lower() in ("all", "mosaic"):
            return _play_chromium_mosaic(quality=req.quality)
        return _play_chromium(req.video_id, quality=req.quality)
    except RuntimeError as exc:
        msg = str(exc)
        if "PDU not ready" in msg:
            raise HTTPException(status_code=503, detail=msg) from exc
        if "No YouTube ID" in msg or "no mosaic" in msg.lower() or "unknown mosaic" in msg.lower():
...
@app.post("/api/mosaic/tab")
def mosaic_tab(req: MosaicTabIn):
    """Turn one of the four Chrome tabs on (YouTube) or off (about:blank)."""
    try:
        with _lock:
            quality = str(_state.get("play_quality") or DEFAULT_PLAY_QUALITY or "4k")
        return _set_mosaic_tab(
            index=req.index,
        ...
        if "PDU not ready" in msg:
            raise HTTPException(status_code=503, detail=msg) from exc
        if "unknown mosaic" in msg.lower() or "no mosaic" in msg.lower():
            raise HTTPException(status_code=400, detail=msg) from exc
        raise HTTPException(status_code=502, detail=msg) from exc


@app.post("/api/mosaic/quality")
def mosaic_quality(req: MosaicQualityIn):
    """Change quality on currently ON tabs only; OFF tabs stay blank."""
    try:
        return _reapply_mosaic_quality(quality=req.quality)
    except RuntimeError as exc:
        msg = str(exc)
        if "PDU not ready" in msg:
            raise HTTPException(status_code=503, detail=msg) from exc
...
def disable_streaming():
    ...
    try:
        if mosaic and slots:
            ...
            chrome_ctl.close_extra_pages(keep=1)
            chrome_ctl.blank()
    except Exception:
        pass
    try:
        ...
        pass
    _log_event("control", "Streaming disabled — 4 Chrome tabs kept, all blank")
    return {"ok": True, "streaming_enabled": False, "tabs": len(slots) if mosaic else 1}
