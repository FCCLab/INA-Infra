#!/usr/bin/env bash
# Wait for PDU tunnel, pin central route over the air, then exec the client.
set -euo pipefail
...
# 5G PDU tunnel routing (over-the-air downlink pull). When running as a sidecar
# in an OAI UE pod, PDU_IFACE is the UE's tunnel (e.g. oaitun_ue1) shared via
# the pod netns, and PDU_ROUTE_HOSTS is a CSV of destinations (the central
# hd-stream server) to pin through it.
PDU_IFACE="${PDU_IFACE:-}"
...
log info "entrypoint start (client)"
wait_for_iface "$OTA_IFACE"
wait_for_iface "$METRICS_IFACE"
setup_pdu_routes
check_chrony

exec python3 /app/client/client.py
