#!/usr/bin/env bash
# Wait for the OTA interface, warn on clock skew, then exec the publisher.
set -euo pipefail
...
# 5G PDU tunnel routing (over-the-air uplink). When running as a sidecar in an
# OAI UE pod, PDU_IFACE is the UE's tunnel (e.g. oaitun_ue1) shared via the pod
# netns, and PDU_ROUTE_HOSTS is a CSV of destinations (the analyzer) to pin
# through it so the stream traverses the 5G air interface instead of pod/mgmt net.
PDU_IFACE="${PDU_IFACE:-}"
...
check_chrony() {
  ...
  local offset
  # `|| true` keeps a missing/unreachable chronyd from tripping set -e/pipefail.
...
setup_pdu_routes() {
  ...
  # synchronously so the FIRST RTSP connection already egresses over the air.
...
VIDEO_SOURCE="${VIDEO_SOURCE:-/data/classroom.mp4}"
VIDEO_URL="${VIDEO_URL:-https://github.com/intel-iot-devkit/sample-videos/raw/master/classroom.mp4}"

download_video_if_missing() {
  if [ ! -s "$VIDEO_SOURCE" ] && [[ ! "$VIDEO_SOURCE" =~ ^/dev/video ]]; then
    log info "video source ${VIDEO_SOURCE} missing; downloading from ${VIDEO_URL}..."
    mkdir -p "$(dirname "$VIDEO_SOURCE")"
    if command -v curl >/dev/null 2>&1; then
      curl -L --retry 3 -s -o "$VIDEO_SOURCE" "$VIDEO_URL" || true
    elif command -v wget >/dev/null 2>&1; then
      wget -q -O "$VIDEO_SOURCE" "$VIDEO_URL" || true
    fi
    if [ -s "$VIDEO_SOURCE" ]; then
      log info "video downloaded successfully ($(du -h "$VIDEO_SOURCE" | cut -f1))"
    else
      log warn "failed to download ${VIDEO_URL}; publisher fallback will be attempted"
    fi
  fi
}

log info "entrypoint start (publisher)"
...
exec python3 /app/client/publisher.py
