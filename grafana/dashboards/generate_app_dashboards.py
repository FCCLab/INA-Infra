...
def _latency_source(app_type: str) -> tuple[str, str, str]:
    """origin, extra filter, field predicate for per-UE ping RTT (origin=client)."""
    return (
        "client",
        '  |> filter(fn: (r) => exists r.ue_id and r.ue_id =~ /client-/)\n',
        'r._field == "app_ue_rtt_ms" or r._field == "rtt_ms"',
    )
...
def flux_per_ue_latency(app_type: str) -> str:
    """Per-UE RTT latency (legend: sliceN-<app>-client-M)."""
    origin, extra, field = _latency_source(app_type)
    origin_filter = f'  |> filter(fn: (r) => r.origin == "{origin}")\n' if origin != "any" else ""
    return (
        'from(bucket: "default")\n'
        "  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)\n"
        f'  |> filter(fn: (r) => r._measurement == "application_metrics" and r.app_type == "{app_type}")\n'
...
def flux_avg_latency(app_type: str) -> str:
    """Mean of per-UE RTT latency (equal weight per UE), same layout as throughput."""
    origin, extra, field = _latency_source(app_type)
    origin_filter = f'  |> filter(fn: (r) => r.origin == "{origin}")\n' if origin != "any" else ""
    return (
        'from(bucket: "default")\n'
        ...
        "  |> group()\n"
        '  |> set(key: "_field", value: "Average RTT")\n'
...
def metrics_panels(app_type: str) -> list[dict]:
    ...
    return [
        ...
        timeseries(5, "Per-UE RTT Latency", 0, 16, 12, 8, flux_per_ue_latency(app_type), "ms", interval="1s", span_nulls=False, show_points="always", legend_calcs=["last", "mean", "max"]),
        timeseries(6, "Average RTT Latency", 12, 16, 12, 8, flux_avg_latency(app_type), "ms", interval="1s", span_nulls=False, show_points="always", legend_calcs=["last", "mean", "max"]),
    ]
