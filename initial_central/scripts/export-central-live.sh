...
After export: ./initial_central/scripts/push-central-to-gitea.sh
EOF
}
...
Generated by \`initial_central/scripts/export-central-live.sh\` from kubectl context \`$CTX\`.
...
Re-export after manual changes, then \`./initial_central/scripts/push-central-to-gitea.sh\`.
...
echo "Next: ./initial_central/scripts/push-central-to-gitea.sh"
