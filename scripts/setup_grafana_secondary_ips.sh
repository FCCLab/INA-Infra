#!/usr/bin/env bash
# Grafana site addressing on 10.1.137 (edge).
#
# 1) /32 secondary on CLUSTER_GRAFANA_NODE site NIC (enp7s0):
#      edge-0=.105
...
grafana_mgmt_nexthop() {
  ...
  case "$node" in
    ...
    *)
      echo "error: no mgmt nexthop mapping for Grafana node '${node}'" >&2
      return 1
      ;;
  esac
}
...
main() {
  ...
  if [[ "$skip_routes" -eq 0 ]]; then
    ...
    for cluster in "${clusters[@]}"; do
      ensure_mgmt_route "$(grafana_vip "$cluster")" "$(grafana_mgmt_nexthop "$cluster")"
    done
  fi
