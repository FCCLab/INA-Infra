...
Targets:
  oai-smf       ${OAI_SLICE_DIR}/oai-cn5g-fed/component/oai-smf
  oai-cn5g-fed  ${OAI_SLICE_DIR}/oai-cn5g-fed
  oai-slice     ${OAI_SLICE_DIR}
  gitops        repos/mgmt repos/central-repo … repos/ue-repo
  parent        FCCLab/INA-Infra (updates submodule gitlinks)
...
Examples:
  $(basename "$0")
  $(basename "$0") -m "sync gitops" gitops
  $(basename "$0") --no-commit oai-smf oai-cn5g-fed
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -*)
      ...
      ;;
    *)
      die "unknown target: $1 (use: oai-smf oai-cn5g-fed oai-slice gitops parent)"
      ;;
  esac
done
...
targets=()
if [[ $# -eq 0 ]]; then
  targets=(oai-smf oai-cn5g-fed oai-slice gitops parent)
else
  for t in "$@"; do
    case "$t" in
...
push_repo() {
  ...
  local label="$3"
  local msg="${4:-$(default_commit_msg)}"
  local url
...
stage_gitlink_in_parent() {
  ...
  [[ -d "$REPO_ROOT/$parent_rel" ]] || return 0

  cd "$REPO_ROOT/$parent_rel"
...
if target_enabled oai-smf; then
  ...
  stage_gitlink_in_parent "$FED_REL" "component/oai-smf" "Update oai-smf gitlink for GitHub"
  if [[ "$NO_COMMIT" != "1" ]] && [[ "$DRY_RUN" != "1" ]]; then
    push_repo "$FED_REL" "INA-Infra-oai-cn5g-fed" "oai-cn5g-fed (gitlink)" "$MSG"
  fi
  echo
fi
...
if target_enabled oai-cn5g-fed; then
  ...
  stage_gitlink_in_parent "$SLICE_REL" "oai-cn5g-fed" "Update oai-cn5g-fed gitlink for GitHub"
  if [[ "$NO_COMMIT" != "1" ]] && [[ "$DRY_RUN" != "1" ]]; then
    push_repo "$SLICE_REL" "INA-Infra-oai-slice-implementation" "oai-slice (gitlink)" "$MSG"
  fi
  echo
fi
...
if target_enabled oai-slice; then
  ...
  stage_gitlink_in_parent "." "$SLICE_REL" "Update ${SLICE_REL} gitlink for GitHub"
  if [[ "$NO_COMMIT" != "1" ]] && [[ "$DRY_RUN" != "1" ]]; then
    push_repo "." "INA-Infra" "parent (gitlink)" "$MSG"
  fi
  echo
fi
...
if target_enabled gitops; then
  for cluster in "${ALL_GITOPS_CLUSTERS[@]}"; do
    ...
    stage_gitlink_in_parent "." "repos/${repo_name}" "Update repos/${repo_name} gitlink for GitHub"
  done
  echo
fi
