...
#
#   ./scripts/label_ina_multus_masters.sh [central|regional|edge|ue|all]
set -euo pipefail
...
export REPO_ROOT
export KUBECONFIG="${KUBECONFIG:-$HOME/.kube/config:$HOME/.kube/config-central:$HOME/.kube/config-regional:$HOME/.kube/config-edge:$HOME/.kube/config-ue}"
...
clusters = ["central", "regional", "edge", "ue"] if target == "all" else [target]
for c in clusters:
