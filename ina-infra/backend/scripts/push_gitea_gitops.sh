#!/usr/bin/env bash
# Push cluster GitOps repos (repos/*) to local Gitea (Config Sync) + GitHub origin (primary).
...
For each repos/<cluster-repo>/ submodule: pull GitHub origin, commit, push to local Gitea (GitOps) + GitHub primary.
...
Per repo:
  ...
  4. git push origin (GitHub primary)
  5. git push gitea (local Gitea GitOps sync for Config Sync)
...
if [[ "$PULL_ONLY" == "1" ]]; then
  echo "Pull repos/ from GitHub origin: ${clusters[*]}"
else
  echo "Push GitOps repos/ to Local Gitea (${GITEA_HOST}:${GITEA_PORT}) + GitHub primary: ${clusters[*]}"
fi
echo
