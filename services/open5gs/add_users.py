...
def read_from_db(db_file):
    ...
    for line in db_file:
        if line.startswith("#"):
            ...
            try:
                name, imsi, key, op_type, op_c, amf, qci, ip_alloc, sst, sd, dnn, session_type = line.strip().split(
                    ',')
            ...
            subscriber_db.append({"imsi": imsi, "key": key, "op": op,
                                  "opc": opc, "amf": amf, "qci": qci, "ip_alloc": ip_alloc.rstrip(), "sst": sst, "sd": sd, "dnn": dnn, "session_type": session_type})
...
def read_from_string(sub_data):
    ...
    Example string: "001010123456780,00112233445566778899aabbccddeeff,opc,63bfa50ee6523365ff14c1f45f88737d,8000,9,10.45.1.2,1,ffffff"
    '''

    subscriber_db = []

    try:
        imsi, key, op_type, op_c, amf, qci, ip_alloc, sst, sd = sub_data.split(
            ',')
    except ValueError as e:
    ...
    subscriber_db.append({"imsi": imsi, "key": key, "op": op,
                          "opc": opc, "amf": amf, "qci": qci, "ip_alloc": ip_alloc.rstrip(), "sst": sst, "sd": sd})
