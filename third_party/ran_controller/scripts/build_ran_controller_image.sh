...
# Source: third_party/ina-infra-oai-ran-controller (FCCLab fork of nephio-project/oai).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
SRC="${RAN_CONTROLLER_SRC:-$REPO_ROOT/third_party/ina-infra-oai-ran-controller}"
...
if [[ ! -f "$SRC/Dockerfile" ]]; then
  echo "error: RAN controller source not found at $SRC (init submodule: git submodule update --init third_party/ina-infra-oai-ran-controller)" >&2
  exit 1
fi
