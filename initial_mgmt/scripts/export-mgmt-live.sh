...
After export: ./initial_mgmt/scripts/push-mgmt-to-gitea.sh
EOF
}
...
Generated by \`initial_mgmt/scripts/export-mgmt-live.sh\` from kubectl context \`$CTX\`.
...
Re-export after manual changes, then \`./initial_mgmt/scripts/push-mgmt-to-gitea.sh\`.
EOF

echo ""
echo "Done: $(find "$OUT_DIR" -name '*.yaml' | wc -l) YAML files in $OUT_DIR"
echo "Next: ./initial_mgmt/scripts/push-mgmt-to-gitea.sh"
